...
class Renderer(nn.Module):
    ...
    def forward(
        ...
        if self.ocl_net is None and render_mode == SAMPLE_IMAGE and self_occlusion:
            self.ocl_net = ras.OcclusionDetector(
                face, height, width, vertex.shape[2], self.block_size
            )
        ...
        if (
            self.diff_raster_net is None
            and render_mode == RASTERIZE_DIFFERENTIABLE_IMAGE
        ):
            self.diff_raster_net = ras.DifferentiableRasterizer(
                face, height, width, self.block_size
            )

        # cal normal
        face_normal, face_area = geo.calc_face_normal(vertex, face)
        vertex_normal = geo.calc_vertex_normal(
            face_normal, face_area, self.vertex_to_face
        )
        ...
        if render_mode == SAMPLE_IMAGE:
            ...
            if self_occlusion:
                occlusion = self.ocl_net(
                    projected_vertex, projected_vertex, vertex, face_normal, R, T
                )
            else:
                occlusion = (rotated_vertex_normal[:, 2, :] < 0).float()

        elif render_mode == RASTERIZE_IMAGE:
            raster_image, raster_mask = self.raster_net(
                projected_vertex, shaded_color, vertex, face_normal, R, T
            )

        elif render_mode == RASTERIZE_DIFFERENTIABLE_IMAGE:
            raster_image, raster_mask = self.diff_raster_net(
                projected_vertex, shaded_color, vertex, face_normal, R, T
            )
