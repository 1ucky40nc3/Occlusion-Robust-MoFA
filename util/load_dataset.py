...
def write_5lms(points, txt_temp):
    ...
    # For 5 landmarks:
    anno_file.write(
        str(round(left_eye_x))
        + "	"
        + str(round(left_eye_y))
        + "\n"
        + str(round(right_eye_x))
        + "	"
        + str(round(right_eye_y))
        + "\n"
        + str(round(nose_xy[0]))
        + "	"
        + str(round(nose_xy[1]))
        + "\n"
        + str(round(left_mouth_xy[0]))
        + "	"
        + str(round(left_mouth_xy[1]))
        + "\n"
        + str(round(right_mouth_xy[0]))
        + "	"
        + str(round(right_mouth_xy[1]))
    )
    anno_file.close()
    ...
    def __getitem__(self, index):
        ...
        if self.test_mode:
            ...
            skin_vis_mask_path = (
                ...
                .replace(".png", ".jpg")
                .replace(".jpg", "_visible_skin_mask.png")
            )
            if os.path.exists(skin_vis_mask_path):
                raw_gtmask = Image.open(skin_vis_mask_path)
                _, gt_mask, _, _ = align_img(
                    raw_gtmask, raw_lm, self.lm3d_std, mask=None
                )
