...
def convert_params(
    ...
    vertex = obj.shape_mean.view(1, 3, -1) + torch.sum(
        obj.shape_basis.view(1, obj.shape_dim, 3, -1)
        * shape_param.view(-1, obj.shape_dim, 1, 1),
        dim=1,
    )
    vertex += obj.exp_mean.view(1, 3, -1) + torch.sum(
        obj.exp_basis.view(1, obj.exp_dim, 3, -1)
        * exp_param.view(-1, obj.exp_dim, 1, 1),
        dim=1,
    )

    color = obj.color_mean.view(1, 3, -1) + torch.sum(
        obj.color_basis.view(1, obj.color_dim, 3, -1)
        * color_param.view(-1, obj.color_dim, 1, 1),
        dim=1,
    )
